...
const delRestaurantById = async (id: number): Promise<ApiResponse<Restaurant[]>> => {
    return HttpRequest.networkRequest({ url: baseUrl + 'restaurant', method: HttpRequest.HttpMethod.DELETE, queryParams: { id: id } });
}

export default { getRestaurants, resetServer, createRestaurant, getRestaurantById, delRestaurantById }